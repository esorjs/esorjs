...
function areValuesEqual(a, b) {
  ...
  if (Array.isArray(a) && Array.isArray(b))
    return a.length === b.length && a.every((v, i) => areValuesEqual(v, b[i]));
  if (typeof a === "object") {
...
function ddiff(parent, current, newList, refNode) {
  ...
  while (aStart < aEnd || bStart < bEnd) {
    ...
    if (aStart === aEnd) {
      ...
      parent.insertBefore(newList[bStart++], current[aStart++].nextSibling);
      ...
      if (index == null) parent.removeChild(current[aStart++]);
        ...
        if (sequence > index - bStart) {
          ...
          while (bStart < index) parent.insertBefore(newList[bStart++], node);
        } else parent.replaceChild(newList[bStart++], current[aStart++]);
      } else aStart++;
    }
  }
}
